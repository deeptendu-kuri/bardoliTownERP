...
: "${SEC:?set SEC (service/secret API key, for creating the admin)}"
ADMIN_EMAIL="${ADMIN_EMAIL:-admin@bardolitown.com}"
...
sql "delete from role_allowlist; insert into role_allowlist(email,role) values ('${ADMIN_EMAIL}','admin');"
...
sql "update profiles p set role='admin', onboarded=true, full_name=coalesce(nullif(full_name,''),'Admin') from auth.users u where u.id=p.id and u.email='${ADMIN_EMAIL}';"
